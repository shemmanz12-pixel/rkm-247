...
// Usage: BASE_URL=https://www.yoursite.com node dist-cleanup.js /path/to/dist
...
async function processFile(file) {
  ...
  if (finalCanonicals.length > 1) {
    ...
    html = html.replace(canonicalRegex, (m, i) => (i === 0 ? m : ''));
  }

  // Ensure the remaining canonical has trailing slash
  const remaining = [...html.matchAll(canonicalRegex)].map(m=>m[0])[0];
  if (remaining) {
