...
// --- 2. THE TOWNS DATA ---
// NOTE: Copy your 'towns' object from townConfig.ts and paste it here
export const towns = {
  // --- TIER 1: MAJOR HUBS ---
  'coalville': {
    name: "Coalville",
...
const services = ['local-plumber', 'emergency-plumber', 'drain-unblocking', 'heating-engineer', 'leak-detection'];
...
// Ensure public/index.html exists before any read operations
(function ensurePublicIndexExists(){
  ...
  try {
    ...
    if (!fs.existsSync(publicIndex)) {
      if (fs.existsSync(rootIndex)) {
        ...
        console.log('generate-seo: copied root index.html -> public/index.html');
      } else {
        const fallback = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>RKM</title></head><body><div id="root"></div></body></html>`;
        fs.writeFileSync(publicIndex, fallback, 'utf8');
        console.log('generate-seo: created minimal public/index.html fallback');
      }
    }
  } catch (e) {
    console.warn('generate-seo: failed to ensure public/index.html', e.message);
  }
})();

// Ensure public index exists and redirect any accidental reads of dist/index.html to public/index.html
(function ensurePublicIndexRedirect(){
  ...
  try {
    ...
    if (!fs.existsSync(publicIndex) && fs.existsSync(templateIndex)) fs.copyFileSync(templateIndex, publicIndex);

    const origRead = fs.readFileSync.bind(fs);
    fs.readFileSync = function(filePath, ...args) {
      try {
        const fp = String(filePath);
        if (fp.endsWith(path.join('dist', 'index.html')) || fp.includes('/dist/index.html') || fp.includes('\\dist\\index.html')) {
          return origRead(publicIndex, ...args);
        }
      } catch (e) {
...
// Robust FS patch: redirect reads of dist/index.html to public/index.html for sync, async (callback), and promise APIs
(function patchFsReadsForDistIndex(){
  try {
    const publicIndex = path.join(process.cwd(), 'public', 'index.html');
    const templateIndex = path.join(__dirname, '..', 'index.html');
    if (!fs.existsSync(path.join(process.cwd(), 'public'))) fs.mkdirSync(path.join(process.cwd(), 'public'), { recursive: true });
    ...
    const origReadSync = fs.readFileSync.bind(fs);
    const origRead = fs.readFile.bind(fs);
    const origPromRead = fs.promises && fs.promises.readFile ? fs.promises.readFile.bind(fs.promises) : null;

    function isDistIndex(p) {
      try {
        const fp = String(p);
        const normalized = fp.split(path.sep).join('/');
        return normalized.endsWith('/dist/index.html') || normalized.includes('/dist/index.html');
      } catch (e) { return false; }
    }

    fs.readFileSync = function(filePath, ...args) {
      if (isDistIndex(filePath)) return origReadSync(publicIndex, ...args);
      return origReadSync(filePath, ...args);
    };

    fs.readFile = function(filePath, ...args) {
      // callback is last arg
      if (isDistIndex(filePath)) return origRead(publicIndex, ...args);
      return origRead(filePath, ...args);
    };

    if (origPromRead) {
      fs.promises.readFile = function(filePath, ...args) {
        if (isDistIndex(filePath)) return origPromRead(publicIndex, ...args);
        return origPromRead(filePath, ...args);
      };
    }

    console.log('generate-seo: patched fs read methods to map dist/index.html -> public/index.html');
  } catch (e) {
    // non-fatal
  }
})();
...
function generate() {
  ...
  Object.entries(towns).forEach(([key, data]) => {
    services.forEach(service => {
      const folderPath = path.join(DIST_PATH, service, key);
      if (!fs.existsSync(folderPath)) fs.mkdirSync(folderPath, { recursive: true });

      const serviceName = service.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');

      // The Massive Unique Block with \n (Newlines) for high-line-count diffing
   // 1. THE MASSIVE UNIQUE BLOCK (Forced Multi-Line)
      const seoBlock = [
        "",
        "",
        `<div id="seo-static-content" style="display:none !important;" aria-hidden="true">`,
        `<h1>${serviceName} in ${data.name}</h1>`,
        `<p>Unique Local Data for ${data.name}: ${data.propertyAgeProfile}</p>`,
        `<p>Common ${serviceName} issues near ${data.landmark} and ${data.road}.</p>`,
        `<ul>`,
        ...(data.housingTypes || []).map(h => `  <li>${h}</li>`),
        `</ul>`,
        `<ul>`,
        ...(data.commonProblems || []).map(p => `  <li>${p}</li>`),
        `</ul>`,
        ...(data.authorityParagraphs || []).map(p => `<p>${p}</p>`),
        `<p>Service area: ${(data.postcodes || []).join(', ')}</p>`,
        `</div>`,
        "",
        ""
      ].join('\n'); // This joins every single element above with a physical New Line

      // 2. BULLETPROOF SPLIT-AND-JOIN INJECTION
      // We split exactly at the root div and join using the seoBlock
      // This ensures the seoBlock is not "stuck" to the minified code on the same line.
      const parts = template.split('<div id="root"></div>');
      let finalHtml = parts[0] + '<div id="root"></div>' + seoBlock + parts[1];

      // 3. Update the Title Tag
      finalHtml = finalHtml.replace(/<title>.*?<\/title>/, `<title>${serviceName} in ${data.name} | RKM Plumbing</title>`);
    
      // Force update the Title tag
      finalHtml = finalHtml.replace(/<title>.*?<\/title>/, `<title>${serviceName} in ${data.name} | RKM Plumbing</title>`);

      fs.writeFileSync(path.join(folderPath, 'index.html'), finalHtml);
    });
    console.log(`✅ Forced Injection: ${data.name}`);
  });
}
