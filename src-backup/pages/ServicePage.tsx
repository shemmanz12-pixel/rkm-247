...
import { Phone, Calendar, MapPin, Zap, AlertCircle, CheckCircle, Clock, Shield, ArrowRight, Info, Home, Droplet, Thermometer, Construction } from 'lucide-react';
...
const ServicePage = () => {
  const { service, town } = useParams<{ service: string; town: string }>();
  ...
  const townData: ExtendedTownData = { ...defaultTownData, ...towns[townKey] };
  const townName = townData.name;

  const authorityTowns = [
    'coalville', 'ashby-de-la-zouch', 'whitwick', 'ibstock', 
    'hugglescote', 'ravenstone', 'coleorton', 'bardon-hill', 'markfield'
  ];
  ...
  const serviceData: Record<string, { title: string; heroText: string; desc: string }> = {
    'local-plumber': {
      ...
      desc: `Looking for a reliable local plumber in ${townName}? From dripping taps and toilet repairs to full bathroom plumbing, we serve the ${townData.postcodes?.[0]} area with high-quality workmanship. Whether you're on ${townData.road} or in the surrounding villages, we offer transparent pricing and a fast local response.`
    },
    'emergency-plumber': {
      ...
      desc: `Need an emergency plumber in ${townName} right now? We handle burst pipes, leaking tanks, and major flooding 24 hours a day. Operating across ${townData.postcodes?.[0]}, we offer a 60-minute response time with no call-out charge, ensuring your home is safe and dry.`
    },
    'drain-unblocking': {
      ...
      desc: `Struggling with a stubborn blockage off ${townData.road}? We provide specialist high-pressure water jetting and CCTV drain surveys across ${townName}. Our local drainage experts handle everything from root ingress to fatbergs with no call-out fee and fixed pricing.`
    },
    'heating-engineer': {
      ...
      desc: `Is your boiler locking out or radiators staying cold in ${townName}? Our heating specialists provide rapid repairs, boiler servicing, and system powerflushing across the ${townData.postcodes?.[0]} area. We ensure your home stays warm with 24/7 emergency support.`
    },
    'leak-detection': {
      ...
      desc: `Noticing damp patches or a dropping pressure gauge near ${townData.road}? We use thermal imaging and acoustic sensors to pinpoint hidden water leaks in ${townName}. Our "Trace and Access" service helps your insurance claim by finding the leak with minimal disruption.`
    }
  };

  const currentService = serviceData[serviceSlug] || serviceData['emergency-plumber'];
  const canonicalUrl = `https://rkm247.co.uk/${serviceSlug}/${townKey}`;

  const getLocalSpice = () => {
    if (towns[townKey]) {
      return `We are your dedicated local specialists for ${townName}. Whether you're near ${townData.landmark} or situated along ${townData.road}, our team is frequently operating in the ${townData.postcodes?.join(' and ')} areas to ensure a rapid response.`;
    }
    return `We are your local ${townName} plumbing experts, providing fast response times and reliable service to the ${townName} community and surrounding postcodes.`;
  };
  const currentLocalText = getLocalSpice();

  // --- 4. CONSOLIDATED SCHEMA (Resolves Duplicate and Missing Field Errors) ---
  const fullSchema = {
        ...
        "mainEntity": [
          {
            ...
            "acceptedAnswer": {
              "@type": "Answer",
              "text": `In ${townName}, we frequently resolve pipe corrosion near ${townData.landmark} and drainage issues along ${townData.road}.`
            }
          },
          {
            "@type": "Question",
            ...
            "acceptedAnswer": {
              "@type": "Answer",
              "text": `Our engineers are frequently operating across ${townData.postcodes?.[0]}, allowing rapid response times throughout ${townName}.`
            }
          }
        ]
      }
  ...
  // --- 5. SEO & HEAD EFFECTS ---
  useEffect(() => {
    document.title = `${currentService.title} in ${townName} | RKM Plumbing`;
    ...
    metaDesc.setAttribute("content", `${currentService.title} in ${townName}. Fast response near ${townData.landmark}. Serving ${townData.postcodes?.[0]} with no call-out fee.`);
  ...
  }, [serviceSlug, townKey, currentService, townName, canonicalUrl, townData]);

  const activities: { type: string; loc: string; time: string; icon: any }[] = [
    { type: 'Heating Repair', loc: townData.name, time: '25 MINS AGO', icon: Zap },
  ...
  // --- AUTHORITY BUILD: HELPER FUNCTIONS ---
  const ContextualLinks = () => {
    return (
      <div className="bg-slate-50 p-5 rounded-lg border-l-4 border-[#A6892C] my-8 shadow-sm text-left">
        <h4 className="font-bold text-slate-900 mb-2 flex items-center gap-2">
           <Zap className="h-4 w-4 text-[#A6892C]" /> Comprehensive Plumbing Support in {townName}
        </h4>
        <p className="text-gray-700 text-sm leading-relaxed">
          While we are currently addressing {currentService.title.toLowerCase()} issues, our team offers full property care. 
          Many plumbing issues are interconnected; for instance, persistent damp patches may require our specialized <Link to={`/leak-detection/${townKey}`} className="text-[#A6892C] font-semibold hover:underline">leak detection in {townName}</Link> to prevent structural damage. 
          If you are experiencing boiler faults alongside pipe issues, our <Link to={`/heating-engineer/${townKey}`} className="text-[#A6892C] font-semibold hover:underline">heating engineers in {townName}</Link> are available for rapid diagnostics. 
          For urgent, out-of-hours disasters, you can always rely on our <Link to={`/emergency-plumber/${townKey}`} className="text-[#A6892C] font-semibold hover:underline">emergency plumber service in {townName}</Link>.
        </p>
      </div>
    );
  };
      ...
      return (
        <div className="bg-white p-5 rounded-lg border border-slate-100 shadow-sm h-full text-left">
          <h3 className="font-bold text-lg text-slate-900 mb-3 flex items-center gap-2">
            {icon || <CheckCircle className="w-4 h-4 text-[#A6892C]" />} {title}
          </h3>
          <ul className="space-y-2">
            {items.map((item, idx) => (
              <li key={idx} className="flex items-start gap-2 text-sm text-gray-700">
                <span className="w-1.5 h-1.5 rounded-full bg-[#A6892C] mt-2 shrink-0"></span>
                <span>{item}</span>
              </li>
            ))}
          </ul>
      ...
      <div className="mt-12 space-y-10 border-t border-gray-100 pt-10 text-left">
        {(townData.propertyAgeProfile || townData.soilType || townData.waterPressureNotes || townData.floodRisk) && (
          <section className="bg-slate-50 p-6 md:p-8 rounded-2xl border border-gray-200">
            <h2 className="text-2xl font-black text-slate-900 mb-6 flex items-center gap-3">
              <Info className="w-6 h-6 text-[#A6892C]" /> Local Plumbing Infrastructure in {townName}
            </h2>
            <div className="space-y-4 text-gray-700 leading-relaxed">
              {townData.propertyAgeProfile && (
                <div>
                  <h4 className="font-bold text-slate-900">Property Age & Composition</h4>
                  <p className="mb-2">{townData.propertyAgeProfile}</p>
                </div>
              )}
              ...
              {townData.insuranceNotes && (
                <div className="mt-4 bg-white p-4 rounded-lg border-l-4 border-[#A6892C]">
                  <p className="text-sm"><strong>Insurance Note:</strong> {townData.insuranceNotes}</p>
                </div>
              )}
            </div>
          </section>
        ...
        {townData.authorityParagraphs && townData.authorityParagraphs.length > 0 && (
          <section className="space-y-4 text-gray-700 leading-relaxed">
             {townData.authorityParagraphs.map((para: string, index: number) => (
               <p key={index}>{para}</p>
             ))}
          </section>
        )}
        ...
        {townData.commercialAreas && townData.commercialAreas.length > 0 && (
          <div className="bg-slate-900 text-gray-300 p-6 rounded-xl">
             <p className="font-medium">
               <span className="text-white font-bold">Commercial Support:</span> We also provide dedicated commercial plumbing support across {townData.commercialAreas.join(', ')} in {townName}, serving local businesses with minimal downtime.
             </p>
          </div>
        )}
        ...
        {townData.nearbyAreas && townData.nearbyAreas.length > 0 && (
          <div className="pt-6">
             <h3 className="text-lg font-bold text-slate-900 mb-4 uppercase tracking-wider">Serving {townName} and Surrounding Villages</h3>
             <div className="flex flex-wrap gap-3">
               {townData.nearbyAreas.map((area, i) => (
                 <Link key={i} to={`/${serviceSlug}/${slugify(area)}`} className="px-3 py-1.5 bg-slate-100 text-slate-700 text-sm font-medium rounded-full hover:bg-[#A6892C] hover:text-white transition-colors">{area}</Link>
               ))}
             </div>
          </div>
        )}
  ...
  const renderNearbyTowns = () => {
    ...
    return (
      <div className="mt-12 pt-8 border-t border-gray-200 text-left">
        <h4 className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-4">Also covering nearby areas</h4>
        <div className="flex flex-wrap gap-4">
          {nearby.map(t => (
            <Link key={t} to={`/${serviceSlug}/${t}`} className="group flex items-center gap-2 text-slate-700 hover:text-[#A6892C] transition-colors">
              <MapPin className="w-4 h-4 text-gray-400 group-hover:text-[#A6892C]" />
              <span className="font-medium text-sm border-b border-transparent group-hover:border-[#A6892C]">{towns[t].name}</span>
            </Link>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white font-sans">
      <Helmet>
        <title>{`${currentService.title} in ${townName} | RKM Plumbing`}</title>
        <meta name="description" content={`${currentService.title} in ${townName}. Fast response near ${townData.landmark}. Serving ${townData.postcodes?.[0]}.`} />
        <link rel="canonical" href={canonicalUrl} />
        <meta property="og:title" content={`${currentService.title} in ${townName} | RKM Plumbing & Heating`} />
        <meta property="og:description" content={`${currentService.title} in ${townName}. Fast response.`} />
        <meta property="og:url" content={canonicalUrl} />
        <meta property="og:type" content="website" />
        <meta property="og:image" content="https://rkm247.co.uk/rkm-logo-final.webp" />

        {/* FIXED: Moving script INTO Helmet to ensure SPA visibility */}
      ...
      <main className="flex-grow">
        <section className="relative bg-white pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden text-left">
          <div className="container mx-auto px-4 relative z-10">
            <div className="flex flex-col lg:flex-row items-center gap-12">
              <div className="flex-1 text-center lg:text-left">
                <div className="mb-8 flex justify-center lg:justify-start">
                  <img src="/rkm-logo-final.webp" alt="RKM Logo" className="h-24 w-auto object-contain" />
                </div>
                <div className="flex justify-center lg:justify-start mb-6">
                  <div className="inline-flex items-center gap-2 bg-[#A6892C]/10 text-[#A6892C] border border-[#A6892C]/30 px-4 py-1.5 rounded-full text-sm font-bold uppercase tracking-wider shadow-sm">
                    <span className="relative flex h-2 w-2">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#A6892C] opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-2 w-2 bg-[#A6892C]"></span>
                    </span>
                    Live Activity: {townName} ({lastActive} mins ago)
                  </div>
                </div>

                <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-slate-900 mb-6 leading-tight uppercase">
                  {currentService.title} <br/><span className="text-[#A6892C]">IN {townName}</span>
                </h1>
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start w-full max-w-md mx-auto lg:mx-0">
                  <a href="https://calendar.app.google/pbb7EJraxjMQd1xS9" className="w-full sm:w-auto bg-[#A6892C] text-slate-900 hover:bg-[#b5952f] px-8 py-4 rounded-lg font-bold text-lg flex items-center justify-center gap-2 shadow-lg transition-all active:scale-95">
                    <Calendar className="w-5 h-5" /> Book Online Now
                  </a>
                  <a href="tel:01530654062" className="w-full sm:w-auto bg-slate-900 text-white hover:bg-slate-800 px-8 py-4 rounded-lg font-bold text-lg flex items-center justify-center gap-2 transition-all active:scale-95">
                    <Phone className="w-5 h-5" /> 01530 654 062
                  </a>
                </div>
              </div>

              <div className="flex-1 w-full max-w-sm md:max-w-lg mx-auto">
                <img 
                  src="/team-photo.webp" 
                  alt={`${currentService.title} Team`} 
                  width="800"
                  height="600"
                  className="rounded-2xl shadow-2xl border-4 border-white w-full h-auto"
                />
              </div>
            </div>
          </div>
        ...
        <section className="py-8 bg-white border-t border-gray-100">
          <div className="container mx-auto px-4">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-xs font-bold uppercase tracking-widest text-[#A6892C]">Live Local Activity</h2>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {activities.map((act, i) => (
                <div key={i} className="bg-slate-50 p-4 rounded-xl border border-gray-100 flex flex-col gap-1 shadow-sm">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-slate-900 text-sm">{act.type}</span>
                    <act.icon className="w-4 h-4 text-[#A6892C]" />
                  </div>
                  <span className="text-xs text-gray-500 flex items-center gap-1">
                    <MapPin className="w-3 h-3" /> {act.loc}
                  </span>
                  <span className="text-[10px] font-bold text-[#A6892C] mt-1 uppercase tracking-widest">
                    {act.time}
                  </span>
                </div>
              ))}
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-gray-100">
              <div className="text-center mb-10">
                <h2 className="text-2xl md:text-3xl font-black mb-4 text-slate-900 uppercase">Expert {currentService.title} Services in {townName}</h2>
                <p className="text-lg text-gray-700 mb-6 leading-relaxed">{currentService.desc}</p>
                <div className="h-px w-20 bg-[#A6892C] mx-auto my-6"></div>
                <p className="text-gray-500 italic font-medium px-2 mb-6">"{currentLocalText}"</p>
                <div className="flex flex-wrap justify-center gap-4">
                  <div className="flex items-center gap-2 px-4 py-2 border rounded-lg bg-white">
                    <Clock className="w-4 h-4 text-[#A6892C]" />
                    <span className="text-sm font-bold">~60 Minute Response</span>
                  </div>
                  <div className="flex items-center gap-2 px-4 py-2 border rounded-lg bg-white">
                    <Shield className="w-4 h-4 text-[#A6892C]" />
                    <span className="text-sm font-bold">No Call Out Charge</span>
                  </div>
                </div>
              </div>
              ...
              {authorityTowns.includes(townKey) && (
                <section className="mt-12 bg-slate-900 text-white p-8 rounded-xl shadow-lg text-left">
                  <h3 className="text-xl font-bold mb-3 flex items-center gap-2"><CheckCircle className="text-[#A6892C]" /> Trusted in {townName}</h3>
                  <p className="leading-relaxed text-gray-300">We regularly complete repairs near {townData.landmark} and along {townData.road}.</p>
                </section>
              )}
              {renderNearbyTowns()}
            </div>
